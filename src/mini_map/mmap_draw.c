...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/18 18:22:32 by kmordaun          #+#    #+#             */
/*   Updated: 2024/04/18 18:30:26 by kmordaun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mmap_draw_alert(t_game *game, t_vec2 p_pos, t_mmap *mmap)
{
	...
	{
		pos.x = ((mmap->al_pos.x * MAP_S) - \
			(p_pos.x * MAP_S) + (SCR_WIDTH / 2));
		pos.y = ((mmap->al_pos.y * MAP_S) - \
			(p_pos.y * MAP_S) + (SCR_HEIGHT / 2) - 21);
		if (pos.x < 1 || pos.x > SCR_WIDTH - 24 \
			|| pos.y < 1 || pos.y > SCR_HEIGHT - 24)
		{
			if (pos.x < 1)
				pos.x = 1;
...
void	mmap_draw(t_game *game)
{
	...
	if (game->mmap.mm_small == true)
	{
		...
		mmap_draw_walls(game->rt0, game->mmap.tiles, \
			game->player.pos, game->mmap);
