...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/18 19:18:02 by kmordaun          #+#    #+#             */
/*   Updated: 2024/04/18 19:18:08 by kmordaun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fl_animation(t_game *game)
{
	...
	flick = (game->fpsc / 30) % 2 == 0;
	if (game->five_light.show_assistance == true)
	{
		if (flick == true)
			mlx_put_image_to_window(game->app.mlx, game->app.win, \
			game->five_light.assistance_img[0], PANEL_POS_X, PANEL_POS_Y);
		else
			mlx_put_image_to_window(game->app.mlx, game->app.win, \
			game->five_light.assistance_img[1], PANEL_POS_X, PANEL_POS_Y);
	}
	if (game->five_light.finished == true)
	{
		if (flick == true)
			mlx_put_image_to_window(game->app.mlx, game->app.win, \
			game->five_light.win_img[0], PANEL_POS_X, PANEL_POS_Y);
		else
			mlx_put_image_to_window(game->app.mlx, game->app.win, \
			game->five_light.win_img[1], PANEL_POS_X, PANEL_POS_Y);
	}
}
...
void	five_lights(t_game *game)
{
	...
	mlx_put_image_to_window(game->app.mlx, game->app.win, \
		game->five_light.back_img, PANEL_POS_X, PANEL_POS_Y);
	while (++i < 10)
		mlx_put_image_to_window(game->app.mlx, game->app.win, \
			game->five_light.swit_img[game->five_light.state[i]], \
			game->five_light.pos_x[i], game->five_light.pos_y);
	if (game->five_light.broken == true)
	{
		mlx_put_image_to_window(game->app.mlx, game->app.win, \
		game->five_light.lights_img[3], PANEL_POS_X + 150, PANEL_POS_Y + 350);
		mlx_put_image_to_window(game->app.mlx, game->app.win, \
		game->five_light.lights_img[1], PANEL_POS_X + 150, PANEL_POS_Y + 413);
	}
	fl_animation(game);
	fl_input(game);
}
