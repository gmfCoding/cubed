...
/*
 * allocates memory for a new node the next 
 * is used to store acces to all nodes inside open
 * or close and the parent node is used for 
 * when the target is found and can trace back the steps 
 ...
 */
t_vec2	*star_construct_path(t_star_node *end_node, \
...
//	end = game->player.pos;
	...
	game->world->map.tiles[(int)end.x + ((
	int)end.y * game->world->map.width)].vis);
	printf("\nstart = %d,%d : end = %d,%d \n", 
