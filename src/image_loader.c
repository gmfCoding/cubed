...
void	load_image_at_index(t_game *game)
{
	if (game->loaded_index[0] < TEX_ENEMY_START)
	{
		...
		{
			mlx_string_put (game->app.mlx, game->app.win, 400, 900, 0xffffff, g_image_paths[game->loaded_index[0]]);	
			game->textures[game->loaded_index[0]] = texture_load(game->app.mlx, g_image_paths[game->loaded_index[0]]);
		}
	}
