...
/*   Updated: 2024/04/29 19:47:50 by clovell          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "map.h"
#include "state.h"
...
static t_err	setup_entity(t_world *world, t_mod_trigger_area *data, t_entity_2 *ent)
{
...
/*
 * NAME,TARGET,ACTIVE (O | C), XPOS, YPOS, RADIUS PCT, WATCH_MODE (P | E), DIST_MODE (M | E)
...
t_err	mod_gen_ta(char *content, int index, t_world *world, t_map *map)
{
	...
	const int			found = ft_sescanf(content, "%N%s,%s,%c,%u,%u,%u,%c,%c\v",
			sizeof(mod.name), &mod.name, &mod.target,
			&mod.active, &mod.pos.x, &mod.pos.y, &mod.radius, &mod.watch, &mod.dist_mode);
